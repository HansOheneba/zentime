...
export async function getValidTimesFromSchedule(
  ...
  // Try to fetch Google Calendar events, but don't fail if it doesn't work
  let eventTimes: { start: Date; end: Date }[] = [];
  try {
    eventTimes = await getCalendarEventTimes(userId, {
      start,
      end,
    });
  } catch (error) {
    console.log(
      `Warning: Could not fetch calendar events for user ${userId}:`,
      error
    );
    // Continue without calendar blocking - user might not have connected Google Calendar
  }
  ...
  return timesInOrder.filter((intervalDate) => {
    ...
    // If no availabilities for this day, skip it
    if (availabilities.length === 0) {
      return false;
    }
    ...
    return (
      ...
      availabilities.some((availability) => {
        // Check if the event starts at or after the availability start
        // and ends at or before the availability end
        return (
          eventInterval.start >= availability.start && // Event starts within availability
          eventInterval.end <= availability.end // Event ends within availability
        );
      })
    );
  });
...
function getAvailabilities(
  ...
  return dayAvailabilities.map((availability) => {
    ...
    // Convert the UTC date to the user's timezone to get the correct local date
    const localDate = toZonedTime(date, timezone);

    // Create start and end dates in the user's timezone, then convert to UTC
    const start = fromZonedTime(
      setMinutes(setHours(localDate, startHour), startMinute),
      timezone
    );

    const end = fromZonedTime(
      setMinutes(setHours(localDate, endHour), endMinute),
      timezone
    );
